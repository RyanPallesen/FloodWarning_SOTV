...
namespace FloodWarning.Artifacts
{
    ...
    internal class Atlas
    {
        ...
        private static void GlobalEventManager_OnInteractionBegin(On.RoR2.GlobalEventManager.orig_OnInteractionBegin orig, GlobalEventManager self, Interactor interactor, IInteractable interactable, GameObject interactableObject)
        {
            ...
            if (RunArtifactManager.instance && RunArtifactManager.instance.IsArtifactEnabled(Artifacts.Atlas.tempDefs[0]) && interactableObject && interactableObject.GetComponent<PurchaseInteraction>() && interactableObject.GetComponent<PurchaseInteraction>().isShrine && TeleporterInteraction.instance)
            {
